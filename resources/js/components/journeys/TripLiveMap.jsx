...
function pointAlong(latLngs, t) {
    ...
    return { latlng: L.latLng(latLngs[latLngs.length - 1]), heading: 0 };
}
...
/**
 * Stable trip map (Leaflet) — same engine that already draws lanes on checkout.
 * Mounts once; only the car marker moves (RAF). No MapLibre style/layer race.
 */
export default function TripLiveMap({
    ...
    const layersRef = useRef({ route: null, casing: null, a: null, b: null, car: null });
    const routePtsRef = useRef(null);
    const carStateRef = useRef({ progress: 0.14, heading: 0, lastTs: 0 });
    const showCarRef = useRef(showCar);
    const onProgressRef = useRef(onCarProgress);
    const propsRef = useRef({ pickupLabel, dropoffLabel, lat, lng, dropLat, dropLng, carLoopMs });

    propsRef.current = { pickupLabel, dropoffLabel, lat, lng, dropLat, dropLng, carLoopMs };
    ...
    useEffect(() => {
        ...
        const drawRoute = async () => {
            ...
            const end = {
                lat: p.dropLat ?? p.lat + 0.022,
                lng: p.dropLng ?? p.lng + 0.028,
            };
            ...
            const a = L.marker([start.lat, start.lng], {
                icon: pinIconA(p.pickupLabel),
                interactive: false,
            }).addTo(map);

            const b = L.marker([end.lat, end.lng], {
                icon: pinIconB(p.dropoffLabel),
                interactive: false,
            }).addTo(map);

            layersRef.current = { ...prev, casing, route, a, b };
        ...
        const tick = (ts) => {
            ...
            if (!showCarRef.current || !pts?.length) {
                if (layersRef.current.car) {
                    map.removeLayer(layersRef.current.car);
            ...
            const speed = 1 / Math.max(14000, propsRef.current.carLoopMs || 48000);
            state.progress += dt * (speed + Math.sin(ts / 1100) * 0.000012);
            if (state.progress >= 0.9) state.progress = 0.12;
            ...
            state.heading = lerpAngle(state.heading, pos.heading, Math.min(1, dt / 160));
            const rot = car.getElement()?.querySelector('.almajd-car-rot');
        ...
        // One delayed invalidate helps when the panel sizes after paint
        ...
        return () => {
            ...
            layersRef.current = { route: null, casing: null, a: null, b: null, car: null };
            map.remove();
            mapRef.current = null;
    ...
    return (
        <div className={`relative isolate overflow-hidden bg-[#e8e6e1] ${className}`}>
            <div ref={containerRef} className="almajd-route-map absolute inset-0 z-0 h-full w-full" />
        </div>
    );
}
