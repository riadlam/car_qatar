...
import { fadeUp, PILLARS, STATS, toArabicDigits } from './data';
...
function Counter({ to, suffix = '', duration = 2000 }) {
    ...
    return (
        <span ref={ref} className="font-ar-display text-4xl text-ivory sm:text-5xl lg:text-6xl">
...
export default function ExcellenceAr() {
    return (
            ...
            <div className="mx-auto max-w-7xl px-4 sm:px-6">
                <div className="mb-12 text-center sm:mb-20">
                    ...
                    <motion.h2 variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }} className="font-ar-display text-3xl leading-tight text-ivory sm:text-4xl lg:text-5xl">
                        نقلٌ يليق <span className="gold-text">بقطر.</span>
                    </motion.h2>
                </div>

                <div className="mb-12 grid grid-cols-2 gap-6 sm:mb-24 sm:gap-8 lg:grid-cols-4">
                    {STATS.map((s, i) => (
                        <motion.div key={s.label} variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="text-center">
                            <Counter to={s.to} suffix={s.suffix} />
                            <p className="mt-2 font-ar-kufi text-[13px] text-ivory/50 sm:mt-3 sm:text-sm">{s.label}</p>
                        </motion.div>
                    ))}
                </div>
                ...
                <motion.div
                    ...
                    className="grid grid-cols-1 gap-px overflow-hidden rounded-2xl border border-gold-500/10 bg-gold-500/10 text-right sm:grid-cols-2 lg:grid-cols-4"
                >
                    {PILLARS.map((p, i) => (
                        <motion.div key={p.title} variants={fadeUp} className="group bg-ink-soft p-6 transition-colors duration-500 hover:bg-wine-950 sm:p-8">
                            ...
                            <h3 className="mt-5 font-ar-display text-xl text-ivory sm:mt-6 sm:text-2xl">{p.title}</h3>
                            <p className="mt-3 font-ar-body text-[15px] leading-loose text-ivory/55">{p.copy}</p>
                        </motion.div>
                    ))}
                </motion.div>
            </div>
